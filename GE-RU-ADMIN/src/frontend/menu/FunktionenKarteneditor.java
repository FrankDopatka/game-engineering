...
public class FunktionenKarteneditor {
	...
	public void neueKarte() {
		...
		final JLabel weltraumKartenArt = new JLabel(); // Universums KartenArt setzen
		final JLabel weltraumFeldArt = new JLabel(); // Standardfeld
		...
		if ((eingabe.getValue() != null) && (eingabe.getValue().equals(JOptionPane.OK_OPTION))) {
			try {
				...
				//Erzeugt ein neues MEnuRechts fuer die neue Karte
	...
	// uebergibt die moeglichen IDs fuer neues Universem
	private Integer[] getMoeglicheUniversumsIds(){
		int maxNummern = 999;
		int zaehler = 0;
		Integer[] idNummern;

		// laden der moeglichen Auswahl von Universen aus der Server Liste
		...
		if (liste != null) {
			...
			if (liste != null) {
				for (D d_Auswahl : liste) {
					if (d_Auswahl.getString("id")==null)
						belegteID[zaehler] = 0;
					else
	...
	// Berechnung die noch moeglichen KartenIDs im Universum
	...
	public Raum bearbeiteWurmloch(Raum r) {
		...
		if (r.isBearbeitet()) {
			if (r.getBiDirektional() > 0) {
				...
				bearbeitet.setText("Ist ein Uni-direktionals Wurmloch\nKoordinaten: X=" + r.getZielX() + " : Y=" + r.getZielY());
			}
		} else {
			bearbeitet.setText("Noch nicht gesetzt");
		}
		JRadioButton dir = new JRadioButton("Uni-Direktional");
		...
		if (r.getZielX() > 0) {
			zielX = new JSpinner(new SpinnerNumberModel(r.getZielX(), 1, frontend.getUniversum().getGroesseX(), 1)); 
			zielY = new JSpinner(new SpinnerNumberModel(r.getZielY(), 1, frontend.getUniversum().getGroesseX(), 1)); 
		} else {
			zielX = new JSpinner(
					new SpinnerNumberModel(frontend.getUniversum().getGroesseX(), 1, frontend.getUniversum().getGroesseX(), 1)); 
			zielY = new JSpinner(
					new SpinnerNumberModel(frontend.getUniversum().getGroesseY(), 1, frontend.getUniversum().getGroesseX(), 1));
		}
		...
		if ((eingabe.getValue() != null) && (eingabe.getValue().equals(JOptionPane.OK_OPTION))) {
			try {
				...
				JOptionPane.showMessageDialog(frontend, "Bitte gueltige Zahlen eingeben!", "Neues Wurmloch erzeugen...",
						JOptionPane.WARNING_MESSAGE);
				e.printStackTrace();
			} catch (Exception e) {
	...
	public void speichernUniversum() {
		...
		if (frontend.getUniversum() != null) {
			...
			frontend.log("Speichern war erfolgreich...");
	...
	public void ladenUniversum() {
		...
		//liste befuellen fuer Auswahl im Dialog
		if (liste != null) {
			for (D d_Auswahl : liste) {
		...
		if ((eingabe.getValue() != null) && (eingabe.getValue().equals(JOptionPane.OK_OPTION))) {
			try {
				...
				JOptionPane.showMessageDialog(frontend, "Ihre Eingaben sind ungueltig:\n"+e.getMessage(), "Neue Karte laden...",JOptionPane.WARNING_MESSAGE);
				e.printStackTrace();
			}
		}
	}
	...
	public boolean ladenKarte(String kArt) {
		...
		chooser.setCurrentDirectory(new File(Parameter.pfadUniversumsDaten));
