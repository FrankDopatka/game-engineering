...
public class Xml {
	public static final String xmlHeader = "<?xml";
	...
	private static Pattern pattern02 = Pattern.compile("</properties>" + System.lineSeparator() + "<properties>",
			Pattern.DOTALL);

	public static D toD(String xml) {
		try {
			if (!xml.startsWith("<?xml"))
				xml = xmlHeaderProperty + xml;
	...
	public static ArrayList<ArrayList <D>> toDArray(String xml) {
		try {
			String[] dArry = xml.split(xmlHeader);
			ArrayList<ArrayList <D>> dAll = new ArrayList<ArrayList <D>>();
			
			if (dArry != null) {
				for (int i = 1; i < dArry.length; i++) {
					dArry[i] = xmlHeader + dArry[i];
					if(dArry[i].endsWith("<?")){
						dArry[i] = dArry[i].substring(0, dArry[i].length()-2);
					}
					dAll.add(toArray(dArry[i]));
				}
				return dAll;
			}
			return null;
